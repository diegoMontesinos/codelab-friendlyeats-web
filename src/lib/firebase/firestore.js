...
const updateWithRating = async (
  ...
  const newNumRatings = data?.numRatings
    ? data.numRatings + 1
    : 1;
  const newSumRating =
...
export async function addReviewToRestaurant(
  ...
  if (!review) {
    throw new Error(
      'A valid review has not been provided.'
    );
  }
...
export function getRestaurantsSnapshot(cb, filters = {}) {
  if (typeof cb !== 'function') {
    console.log(
      'Error: The callback parameter is not a function'
    );
    return;
  }
...
export async function getReviewsByRestaurantId(
  ...
  if (!restaurantId) {
    console.log(
      'Error: Invalid restaurantId received: ',
      restaurantId
    );
    return;
  }
